 ...
 * 取得する。GetUserはユーザー自身のアクセストークンを渡すだけで呼べる操作でAWS SigV4署名は不要なため、 {@link
 * com.ieltscreator.api.common.config.CognitoIdentityProviderClientConfig}が
 * AnonymousCredentialsProviderで構成した共有Beanをそのまま再利用する（呼び出しにはApp Clientのスコープに
 * aws.cognito.signin.user.admin が必要。infraリポジトリ terraform/modules/cognito 参照）。
 */
@Component
